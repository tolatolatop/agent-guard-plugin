...
def decide_write(state: dict[str, Any], target_path: str) -> dict[str, str]:
    ...
    if state["stage"] == "RED_TEST" and normalized.startswith("src/"):
    ...
    if matches_any(normalized, state.get("forbidden_paths", [])):
        return blocked(f"Path {normalized} matches forbidden path policy for stage {state['stage']}.")
    ...
    if allowed_paths and not matches_any(normalized, allowed_paths):
        return blocked(f"Path {normalized} is outside allowed paths for stage {state['stage']}.")

    return allowed(f"Path {normalized} is allowed during {state['stage']}.")
