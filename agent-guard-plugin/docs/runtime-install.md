...
- In this mode the installer only writes project files such as `.claude/settings.local.json`, `.claude/skills/`, `.opencode/plugins/`, `.opencode/skills/`, or `.codex/hooks.json`.
...
- Codex: `.codex/hooks.json` and `.agent-guard/skills/*.md`
...
- Claude Code and OpenCode both get native skill layouts so their applications can discover the workflow bundle without relying on a private flat directory convention.
